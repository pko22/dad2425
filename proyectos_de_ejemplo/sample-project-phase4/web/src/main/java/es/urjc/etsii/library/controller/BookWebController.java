package es.urjc.etsii.library.controller;
...
import es.urjc.etsii.library.BookDto;
import es.urjc.etsii.library.model.Book;
import es.urjc.etsii.library.service.BookService;
import es.urjc.etsii.library.service.ShopService;
...
public class BookWebController {
	...
	public String newBookProcess(Model model, Book book, MultipartFile imageField, @RequestParam List<Long> selectedShops) throws IOException {
		...
		// Filter the description using the internal service for filtering
		RestTemplate restTemplate = new RestTemplate();
		Description descriptionFiltered = null;
		try {
			descriptionFiltered = restTemplate.postForObject("http://"+ servicioInternoRest + ":8080/filter", new Description(book.getDescription()), Description.class);
		} catch(RestClientException e) {
			System.out.println("Error calling rest server");
		}
		
		if(descriptionFiltered != null) {
			book.setDescription(descriptionFiltered.getText());
		}
		
		bookService.save(book);

		// Send notification
		rabbitTemplate.convertAndSend("notifications", new BookDto(book.getTitle(), book.getDescription()));
